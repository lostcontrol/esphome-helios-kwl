...
constexpr uint8_t CELSIUS_TO_NTC_MAP[256] = {
  // -128 to -113
...
const int HeliosKwlComponent::NTC_TO_CELSIUS[] = {
...
void HeliosKwlComponent::control_bypass_operating_temperature(uint8_t c) {
...
void HeliosKwlComponent::control_max_fan_speed(uint8_t s) {
  uint8_t mask = s == 0 ? 0 : static_cast<uint8_t>((1u << std::min(static_cast<uint8_t>(8), s)) - 1u);
  set_value(0xA5, mask);
}
...
float HeliosKwlComponent::ntc_to_celsius(uint8_t n) {
  return NTC_TO_CELSIUS[n];
}

uint8_t HeliosKwlComponent::celsius_to_ntc(int8_t c) {
  return CELSIUS_TO_NTC_MAP[static_cast<uint8_t>(c)];
}
